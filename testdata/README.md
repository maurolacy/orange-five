...
|---|---|---|
| `ref1.png` … `ref10.png` | ~0.2–1M each | Reference frames from WNT broadcast (bright studio). Copied from the `orange-five-detect` repo. refs 1–3: overhead/high angle, table fills frame; refs 4–7, 9, 10: higher/lateral; ref8: distant, table is a small top-band wedge. |
| `table_fail1.png` | ~180K | **Failure frame 1**: dark arena. Bed felt at L≈0.12, spotlight gradient, player's grey shirt near the slate. Originally: player welded to the table mask. Known-limited: detector catches only a bed sliver (~5%). |
...
originals are kept OUTSIDE the repo in `~/Projects/pool-color-restorer-originals/`):

- `ref1–4, ref8` at native ≤2560-wide — the Rust-baseline working space
  (`detector.test.js` needs ~25–35 px ball radii; its blob gates are absolute
  px). `loadWorking(name, 2560)` is a no-op for these.
- Everything else at 480-wide — the table detector's working resolution
  (`loadWorking(name)` is a no-op for these).
- If you ever change resolutions, re-encode in ONE box-filter step from the
  originals using `tests/helpers.downscaleRgba` — two-step resampling changes
  the pixels the gates see.
